...
class DatabaseManager:
    ...
    @staticmethod
    def _prepare_asyncpg_ssl(database_url: str) -> tuple:
        """Strip SSL-related query parameters from the URL and return proper connect_args.

        asyncpg does not accept 'ssl', 'sslmode', or 'channel_binding' as URL query
        parameters. Instead, SSL must be configured via the `ssl` connect_arg with a
        proper SSLContext. This method:
        1. Parses and removes ssl/sslmode from the URL query string
        2. Returns an appropriate connect_args dict with an SSLContext if SSL was requested
        ...
        for key in ssl_keys:
            if key in params:
                val = params[key][0].lower() if params[key] else ""
                if val in ("require", "true", "verify-ca", "verify-full", "prefer", "1", "yes"):
                    ssl_requested = True
                del params[key]

        # Also remove channel_binding if present in URL
        ...
        connect_args = {}
        if ssl_requested:
            # Create a permissive SSL context (equivalent to sslmode=require)
            ssl_ctx = ssl.create_default_context()
            ssl_ctx.check_hostname = False
            ssl_ctx.verify_mode = ssl.CERT_NONE
            connect_args["ssl"] = ssl_ctx
            logger.info("Configured asyncpg SSL via SSLContext (sslmode=require equivalent)")
    ...
    async def init_db(self):
        ...
        try:
            ...
            # Strip SSL query params and build proper connect_args for asyncpg
            # This avoids the 'channel_binding' error in asyncpg
            database_url, connect_args = self._prepare_asyncpg_ssl(database_url)
