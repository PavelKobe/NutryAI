...
async function uploadPhotoToStorage(
  ...
  const res = await client.api.post('/api/v1/storage/upload-url', {
  ...
  const dlRes = await client.api.post('/api/v1/storage/download-url', {
    bucket_name: BUCKET_NAME,
    object_key: objectKey,
  });
    ...
    try {
      ...
      await client.ai.gentxt({
        messages: [
          {
            role: 'system',
            content: `Ты — эксперт по распознаванию еды. Проанализируй фото и определи блюдо.
    ...
    try {
      ...
      if (photoFile) {
        try {
          ...
          const userRes = await client.api.get('/api/v1/auth/me');
