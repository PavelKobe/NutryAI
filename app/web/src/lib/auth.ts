...
class RPApi {
  ...
  async getCurrentUser() {
    try {
      ...
      throw new Error(ax.response?.data?.detail || 'Failed to get user info');
    }
  }
  ...
  async login() {
    try {
      ...
      const ax = error as AxiosError<{ detail?: string }>;
      throw new Error(ax.response?.data?.detail || 'Failed to initiate login');
    }
  }
  ...
  async logout() {
    try {
      ...
      const ax = error as AxiosError<{ detail?: string }>;
      throw new Error(ax.response?.data?.detail || 'Failed to logout');
    }
  }
}
